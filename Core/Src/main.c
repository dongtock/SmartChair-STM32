...
// =========================================================================
// 12채널 HX711 병렬 — Blocking (초기화/캘리브레이션/착석감지 전용)
// =========================================================================
...
void Run_Calibration(void)
{
  ...
  for (int cnt = 0; cnt < 500; cnt++)
  {
    ...
    if (rdy) { vl53l8cx_get_ranging_data(&vl53l8cx_dev[0], &tof3d_results);
      for (int j=0;j<16;j++) pkt.tof3d[j]=(uint16_t)tof3d_results.distance_mm[VL53L8CX_NB_TARGET_PER_ZONE*j]; }
    rdy=0; vl53l8cx_check_data_ready(&vl53l8cx_dev[1], &rdy);
    if (rdy) { vl53l8cx_get_ranging_data(&vl53l8cx_dev[1], &tof3d_results);
      for (int j=0;j<16;j++) pkt.tof3d[16+j]=(uint16_t)tof3d_results.distance_mm[VL53L8CX_NB_TARGET_PER_ZONE*j]; }

    for (int i=0;i<4;i++) {
      MUX_SelectChannel(i+2); uint8_t is=0;
      VL53L0X_RdByte(&vl53l0x_dev[i],0x13,&is);
      if (is&0x07) { uint8_t buf[12];
    ...
    for (int i=0;i<2;i++) {
      MUX_SelectChannel(i+6); uint8_t b[6]; int16_t ay,az;
      if (HAL_I2C_Mem_Read(&hi2c1,MPU6050_ADDR,0x3B,1,b,6,20)==HAL_OK) {
        ay=(b[2]<<8)|b[3]; az=(b[4]<<8)|b[5];
        pkt.mpu[i]=(int16_t)(atan2((float)ay,(float)az)*(180.0/3.141592));
      } else pkt.mpu[i]=0;
    }

    int32_t raw[12]={0,}; HX711_ReadAll_Blocking(raw);
    for (int i=0;i<12;i++) pkt.hx711[i]=raw[i]-hx711_offset[i];

    uint8_t *p=(uint8_t*)&pkt; uint8_t ck=0;
    ...
    { static uint32_t ot=0;
      if (oled_ok && HAL_GetTick()-ot>=1000) {
        char ob[19]; snprintf(ob,sizeof(ob),"CAL: %d/%d",cnt,500);
        OLED_Line(1,ob); OLED_Show(); ot=HAL_GetTick();
      }
    }
...
int main(void)
{
  ...
  { char ob[19]; snprintf(ob,sizeof(ob),"3D: %s %s",tof3d_ok[0]?"OK":"ER",tof3d_ok[1]?"OK":"ER");
    OLED_Line(1,ob); OLED_Show(); }
  HAL_Delay(2000);
  ...
  { char ob[19]; snprintf(ob,sizeof(ob),"1D:%d/4 MPU:%d/2",t1d_cnt,mpu_cnt);
    OLED_Line(2,ob); OLED_Show(); }
  HAL_Delay(3000);
  ...
  while (1) {
    ...
    { char ob[19]; snprintf(ob,sizeof(ob),"OK:%d FAIL:%d",tx_ok,tx_fail);
      OLED_Line(1,ob); OLED_Show(); }
    memset(rx_ack,0,sizeof(rx_ack));
    HAL_UART_Receive(&huart1,(uint8_t*)rx_ack,10,500);
    if (strstr(rx_ack,"ACK")) { OLED_Line(2,"ACK received!"); OLED_Show(); break; }
  }
  HAL_UART_Transmit(&huart1,(uint8_t*)"LINK_OK\n",8,100);
  OLED_Line(3,"LINK_OK sent!"); OLED_Show();
  HAL_Delay(1000);
  OLED_Clear(); OLED_Line(0,"=== INIT OK ==="); OLED_Line(1,"Ready to start"); OLED_Show();
  HAL_Delay(2000);
  ...
  while (1)
  {
    ...
    // =========================================================
    // [2단계] 착석 감지 (좌석 로드셀 ch8~11 합산 > 50000)
    // =========================================================
    ...
    // =========================================================
    // [3단계] CAL vs GO (CHK_SIT 재수신 시 SIT 재응답)
    // =========================================================
    ...
    if (is_seated == 1)
    {
      ...
      uint32_t empty_count=0; uint8_t exit_reason=0;
      ...
      while (is_seated)
      {
        ...
        int32_t raw[12]={0,}; int32_t seat_sum=0;
        if (HX711_ReadAll_Raw(raw)) {
          for (int i=0;i<12;i++) {
            pkt.hx711[i]=raw[i]-hx711_offset[i];
            if (i>=8&&i<=11) seat_sum+=pkt.hx711[i];
          }
        } else {
        ...
        // 체크섬 + 전송
        uint8_t *pb=(uint8_t*)&pkt; uint8_t ck=0;
...
{ uint8_t b[2]={d>>8,d&0xFF}; return VL53L0X_WriteMulti(Dev,idx,b,2); }
VL53L0X_Error VL53L0X_WrDWord(VL53L0X_DEV Dev,uint8_t idx,uint32_t d)
{ uint8_t b[4]={(d>>24)&0xFF,(d>>16)&0xFF,(d>>8)&0xFF,d&0xFF}; return VL53L0X_WriteMulti(Dev,idx,b,4); }
VL53L0X_Error VL53L0X_UpdateByte(VL53L0X_DEV Dev,uint8_t idx,uint8_t a,uint8_t o)
{ uint8_t d; VL53L0X_ReadMulti(Dev,idx,&d,1); d=(d&a)|o; return VL53L0X_WriteMulti(Dev,idx,&d,1); }
...
{ uint8_t b[2]; VL53L0X_ReadMulti(Dev,idx,b,2); *d=((uint16_t)b[0]<<8)|b[1]; return 0; }
VL53L0X_Error VL53L0X_RdDWord(VL53L0X_DEV Dev,uint8_t idx,uint32_t *d)
{ uint8_t b[4]; VL53L0X_ReadMulti(Dev,idx,b,4); *d=((uint32_t)b[0]<<24)|((uint32_t)b[1]<<16)|((uint32_t)b[2]<<8)|b[3]; return 0; }

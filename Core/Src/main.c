 ...
 ******************************************************************************
 * @file           : main.c
 * @brief          : SPCC Production Firmware
 *                   HX711 12채널, 착석감지, STAND 활성화
 *                   CHK_SIT 전체 매칭, GO 매칭, CAL_DONE 100ms 딜레이
 ******************************************************************************
 */
...
int32_t hx711_weight[12] = {0, };
int32_t hx711_offset[12] = {0, };
...
HAL_StatusTypeDef VL53L8CX_Init(uint8_t ch)
{
  ...
  if (ch == 0) {
    vl53l8cx_dev[ch].platform.ncs_port   = TOF3D_1_NCS_GPIO_Port;
    vl53l8cx_dev[ch].platform.ncs_pin    = TOF3D_1_NCS_Pin;
    vl53l8cx_dev[ch].platform.lpn_port   = TOF3D_1_LPN_GPIO_Port;
    vl53l8cx_dev[ch].platform.lpn_pin    = TOF3D_1_LPN_Pin;
    vl53l8cx_dev[ch].platform.pwren_port = TOF3D_1_PWREN_GPIO_Port;
    vl53l8cx_dev[ch].platform.pwren_pin  = TOF3D_1_PWREN_Pin;
  } else {
    vl53l8cx_dev[ch].platform.ncs_port   = TOF3D_2_NCS_GPIO_Port;
    vl53l8cx_dev[ch].platform.ncs_pin    = TOF3D_2_NCS_Pin;
    vl53l8cx_dev[ch].platform.lpn_port   = TOF3D_2_LPN_GPIO_Port;
    vl53l8cx_dev[ch].platform.lpn_pin    = TOF3D_2_LPN_Pin;
    vl53l8cx_dev[ch].platform.pwren_port = TOF3D_2_PWREN_GPIO_Port;
    vl53l8cx_dev[ch].platform.pwren_pin  = TOF3D_2_PWREN_Pin;
  }

  HAL_GPIO_WritePin(vl53l8cx_dev[ch].platform.ncs_port,
                    vl53l8cx_dev[ch].platform.ncs_pin,
                    GPIO_PIN_SET);

  VL53L8CX_Reset_Sensor(&vl53l8cx_dev[ch].platform);

  status = vl53l8cx_is_alive(&vl53l8cx_dev[ch], &is_alive);
  if (status || !is_alive) return HAL_ERROR;

  status = vl53l8cx_init(&vl53l8cx_dev[ch]);
  if (status) return HAL_ERROR;

  status = vl53l8cx_set_resolution(&vl53l8cx_dev[ch], VL53L8CX_RESOLUTION_4X4);
  if (status) return HAL_ERROR;

  status = vl53l8cx_set_ranging_frequency_hz(&vl53l8cx_dev[ch], 50);
  if (status) return HAL_ERROR;

  status = vl53l8cx_start_ranging(&vl53l8cx_dev[ch]);
  if (status) return HAL_ERROR;
...
// =========================================================================
// 12채널 HX711 병렬 — Non-blocking (50Hz 루프 전용)
// DOUT1~6: PA1,PA2,PA3,PA8,PA11,PA12
// DOUT7~12: PB15,PB5,PB8,PB9,PB10,PB12
// =========================================================================
uint8_t HX711_ReadAll_Raw(int32_t *data)
{
  ...
  for (int i = 0; i < 12; i++)
    if (data[i] & 0x00800000) data[i] |= 0xFF000000;

  return 1;
}

// =========================================================================
// 12채널 HX711 병렬 — Blocking (초기화/착석감지 전용)
// =========================================================================
...
void Run_Calibration(void)
{
  ...
  memcpy(pkt.header, "CAL:", 4);

  OLED_Clear();
  OLED_Line(0, "Calibrating...");
  OLED_Show();
  ...
  for (int cnt = 0; cnt < 500; cnt++)
  {
    ...
    if (rdy) {
      vl53l8cx_get_ranging_data(&vl53l8cx_dev[0], &tof3d_results);
      for (int j = 0; j < 16; j++)
        pkt.tof3d[j] = (uint16_t)tof3d_results.distance_mm[VL53L8CX_NB_TARGET_PER_ZONE * j];
    }

    rdy = 0;
    vl53l8cx_check_data_ready(&vl53l8cx_dev[1], &rdy);
    if (rdy) {
      vl53l8cx_get_ranging_data(&vl53l8cx_dev[1], &tof3d_results);
      for (int j = 0; j < 16; j++)
        pkt.tof3d[16 + j] = (uint16_t)tof3d_results.distance_mm[VL53L8CX_NB_TARGET_PER_ZONE * j];
    }

    for (int i = 0; i < 4; i++) {
      MUX_SelectChannel(i + 2);
      uint8_t is = 0;
      ...
      if (is & 0x07) {
        ...
        uint8_t st = buf[0] >> 3;
        if (st == 6 || st == 9 || st == 11) {
          uint16_t d = ((uint16_t)buf[10] << 8) | buf[11];
          if (d < 1200) pkt.tof1d[i] = d;
        }
      }
    }

    for (int i = 0; i < 2; i++) {
      MUX_SelectChannel(i + 6);
      uint8_t b[6];
      int16_t ax, az;
...

    {
      ...
      if (oled_ok && HAL_GetTick() - ot >= 1000) {
        char ob[19];
        snprintf(ob, sizeof(ob), "CAL: %d/%d", cnt, 500);
        OLED_Line(1, ob);
        OLED_Show();
        ot = HAL_GetTick();
      }
...
int main(void)
{
  ...
  if (HAL_I2C_IsDeviceReady(&hi2c1, 0x78, 2, 50) == HAL_OK) {
    ...
    OLED_Line(0, "Booting...");
    OLED_Show();
  } else if (HAL_I2C_IsDeviceReady(&hi2c1, MUX_ADDR, 2, 50) == HAL_OK) {
    i2c_ok = 1;
  }
  ...
  for (uint8_t i = 0; i < 2; i++) {
    if (VL53L8CX_Init(i) == HAL_OK) tof3d_ok[i] = 1;
  }
  {
    char ob[19];
    snprintf(ob, sizeof(ob), "3D: %s %s", tof3d_ok[0] ? "OK" : "ER", tof3d_ok[1] ? "OK" : "ER");
    OLED_Line(1, ob);
    OLED_Show();
  }
  HAL_Delay(2000);
  ...
  if (i2c_ok && HAL_I2C_IsDeviceReady(&hi2c1, MUX_ADDR, 5, 100) == HAL_OK) {
    for (uint8_t i = 2; i < 8; i++) {
      ...
      if (HAL_I2C_IsDeviceReady(&hi2c1, addr, 3, 100) == HAL_OK) {
        if (i < 6) {
          if (VL53L0X_Init(i - 2) == HAL_OK) t1d_cnt++;
        } else {
          if (MPU6050_Init() == HAL_OK) mpu_cnt++;
        }
      }
    }
  }
  {
    char ob[19];
    snprintf(ob, sizeof(ob), "1D:%d/4 MPU:%d/2", t1d_cnt, mpu_cnt);
    OLED_Line(2, ob);
    OLED_Show();
  }
  HAL_Delay(3000);
  ...
  // [단계 4] HX711 Tare
  OLED_Line(3, "HX711 Tare...");
  OLED_Show();
  HX711_Init();
  OLED_Line(3, "HX711: OK");
  OLED_Show();
  HAL_Delay(2000);

  // [단계 5] RPi5 핸드셰이크
  OLED_Clear();
  OLED_Line(0, "RPi5 waiting...");
  OLED_Show();

  char rx_ack[16] = {0};
  uint8_t b = 0;
  uint8_t idx = 0;
  uint16_t tx_ok = 0, tx_fail = 0;

  while (1) {
    HAL_StatusTypeDef tx_result =
        HAL_UART_Transmit(&huart1, (uint8_t*)"READY\n", 6, 100);

    if (tx_result == HAL_OK) tx_ok++;
    else tx_fail++;

    {
      char ob[20];
      snprintf(ob, sizeof(ob), "OK:%d FAIL:%d", tx_ok, tx_fail);
      OLED_Line(1, ob);
      OLED_Show();
    }
    ...
    while (HAL_GetTick() - start < 500) {
      if (HAL_UART_Receive(&huart1, &b, 1, 10) == HAL_OK) {
        if (idx < sizeof(rx_ack) - 1) {
          rx_ack[idx++] = b;
          rx_ack[idx] = '\0';
        }

        if (strstr(rx_ack, "ACK")) {
          OLED_Line(2, "ACK received!");
          OLED_Show();
          goto handshake_done;
        }
      }
...
handshake_done:
  ...
  while (1)
  {
    ...
    // [2단계] 착석 감지
    OLED_Line(0, "Wait SIT...");
    OLED_Show();

    while (1)
    {
      int32_t current_raw[12] = {0,};
      HX711_ReadAll_Blocking(current_raw);
    ...
    if (is_seated == 1)
    {
      Main_Packet_t pkt;
      memset(&pkt, 0, sizeof(pkt));
      memcpy(pkt.header, "DAT:", 4);

      OLED_Clear();
      OLED_Line(0, "Measuring...");
      OLED_Show();

      uint32_t empty_count = 0;
      uint8_t exit_reason = 0;
      ...
      while (is_seated)
      {
        ...
        // [4] HX711 — Non-blocking 12채널
        int32_t raw[12] = {0,};
        int32_t seat_sum = 0;        // offset 뺀 값 합
        int32_t seat_raw_sum = 0;    // raw 값 합 (ch9~12)
        ...
        if (oled_ok && HAL_GetTick() - oled_tick >= 1000) {
          ...
          snprintf(ob, sizeof(ob), "S+TX: %lu ms", total_ms);
          OLED_Line(1, ob);

          snprintf(ob, sizeof(ob), "AVG: %lu Hz", hz_count);
          OLED_Line(2, ob);

          snprintf(ob, sizeof(ob), "RAW:%ld", (long)seat_raw_sum);
          OLED_Line(3, ob);
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOB, TOF3D_1_LPN_Pin | TOF3D_1_PWREN_Pin |
                           TOF3D_2_LPN_Pin | TOF3D_2_PWREN_Pin,
                           GPIO_PIN_RESET);
  HAL_GPIO_WritePin(TOF3D_2_NCS_GPIO_Port, TOF3D_2_NCS_Pin, GPIO_PIN_SET);

  g.Pin = HX_SCK_Pin | TOF3D_1_NCS_Pin;
  g.Mode = GPIO_MODE_OUTPUT_PP;
  g.Pull = GPIO_NOPULL;
  g.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOA, &g);

  g.Pin = DOUT_1_Pin | DOUT_2_Pin | DOUT_3_Pin | DOUT_4_Pin |
          DOUT_5_Pin | DOUT_6_Pin;
  g.Mode = GPIO_MODE_INPUT;
  g.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(GPIOA, &g);

  g.Pin = TOF3D_1_LPN_Pin | TOF3D_1_PWREN_Pin |
          TOF3D_2_NCS_Pin | TOF3D_2_LPN_Pin | TOF3D_2_PWREN_Pin;
  g.Mode = GPIO_MODE_OUTPUT_PP;
  g.Pull = GPIO_NOPULL;
  g.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOB, &g);

  g.Pin = DOUT_11_Pin | DOUT_12_Pin | DOUT_7_Pin |
          DOUT_8_Pin | DOUT_9_Pin | DOUT_10_Pin;
  g.Mode = GPIO_MODE_INPUT;
  g.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(GPIOB, &g);
}
...
/* USER CODE BEGIN 4 */
VL53L0X_Error VL53L0X_WriteMulti(VL53L0X_DEV Dev, uint8_t idx, uint8_t *p, uint32_t c)
{ HAL_I2C_Mem_Write(Dev->I2cHandle, Dev->I2cDevAddr, idx, I2C_MEMADD_SIZE_8BIT, p, c, 100); return 0; }

VL53L0X_Error VL53L0X_ReadMulti(VL53L0X_DEV Dev, uint8_t idx, uint8_t *p, uint32_t c)
{ HAL_I2C_Mem_Read(Dev->I2cHandle, Dev->I2cDevAddr, idx, I2C_MEMADD_SIZE_8BIT, p, c, 100); return 0; }

VL53L0X_Error VL53L0X_WrByte(VL53L0X_DEV Dev, uint8_t idx, uint8_t d)
{ return VL53L0X_WriteMulti(Dev, idx, &d, 1); }

VL53L0X_Error VL53L0X_WrWord(VL53L0X_DEV Dev, uint8_t idx, uint16_t d)
{
  uint8_t b[2] = {d >> 8, d & 0xFF};
  return VL53L0X_WriteMulti(Dev, idx, b, 2);
}

VL53L0X_Error VL53L0X_WrDWord(VL53L0X_DEV Dev, uint8_t idx, uint32_t d)
{
  uint8_t b[4] = {(d >> 24) & 0xFF, (d >> 16) & 0xFF, (d >> 8) & 0xFF, d & 0xFF};
  return VL53L0X_WriteMulti(Dev, idx, b, 4);
}

VL53L0X_Error VL53L0X_UpdateByte(VL53L0X_DEV Dev, uint8_t idx, uint8_t a, uint8_t o)
{
  uint8_t d;
  VL53L0X_ReadMulti(Dev, idx, &d, 1);
  d = (d & a) | o;
  return VL53L0X_WriteMulti(Dev, idx, &d, 1);
}

VL53L0X_Error VL53L0X_RdByte(VL53L0X_DEV Dev, uint8_t idx, uint8_t *d)
{ return VL53L0X_ReadMulti(Dev, idx, d, 1); }

VL53L0X_Error VL53L0X_RdWord(VL53L0X_DEV Dev, uint8_t idx, uint16_t *d)
{
  uint8_t b[2];
  VL53L0X_ReadMulti(Dev, idx, b, 2);
  *d = ((uint16_t)b[0] << 8) | b[1];
  return 0;
}

VL53L0X_Error VL53L0X_RdDWord(VL53L0X_DEV Dev, uint8_t idx, uint32_t *d)
{
  uint8_t b[4];
  VL53L0X_ReadMulti(Dev, idx, b, 4);
